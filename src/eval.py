...
def eval_batch(model, task_sampler, xs, xs_p=None):
    ...
    if torch.cuda.is_available() and model.name.split("_")[0] in ["gpt2", "lstm"]:
        device = "cuda"
    elif torch.backends.mps.is_available() and model.name.split("_")[0] in ["gpt2", "lstm"]:
        device = "mps"
    else:
        device = "cpu"

    # Move xs to device before task.evaluate
    xs = xs.to(device)
    if xs_p is not None:
        xs_p = xs_p.to(device)
    ...
    if xs_p is None:
        ...
        # Compute per-point squared error (mean over feature dim only)
        metrics = (pred.cpu() - ys.cpu()).square().mean(dim=-1)
    else:
        ...
        for i in range(n_points):
            ...
            # Compute per-point squared error (mean over feature dim only)
            metrics[:, i] = (pred.cpu() - ys.cpu()).square().mean(dim=-1)[:, i]
...
def build_evals(conf):
    ...
    # Get kwargs from config and override device
    data_kwargs = dict(conf.training.data_kwargs) if hasattr(conf.training, 'data_kwargs') else {}
    task_kwargs = dict(conf.training.task_kwargs) if hasattr(conf.training, 'task_kwargs') else {}
    data_kwargs['device'] = eval_device
    task_kwargs['device'] = eval_device
    ...
    # Commented out: requires sample_transformation not available in samplers
...
def get_run_metrics(
    ...
    if skip_model_load:
        ...
        model = model.eval()
        all_models = [model]
        if not skip_baselines and hasattr(models, 'get_relevant_baselines'):
